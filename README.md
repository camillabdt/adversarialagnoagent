...
1. A baseline Masquerade fault attack configuration (JSON) is used to generate a synthetic dataset via **ERENO-2.0**.
...
4. Each variant is fed back to ERENO-2.0 to generate a new dataset, which is evaluated against the fixed baseline IDS.
...
- Python 3.10+
- Java (required by ERENO-2.0 internally — check its own README)
...
ERENO-2.0 is included as a git submodule. You **must** use `--recurse-submodules`:
...
Follow the setup instructions inside `ERENO-2.0/` (the submodule has its own README and dependencies). Make sure it runs correctly before starting the pipeline:

```bash
cd ERENO-2.0
# follow ERENO-2.0's own setup steps
cd ..
```
...
```
adversarialagnoagent/
├── ERENO-2.0/               # submodule — synthetic IEC-61850 dataset generator
├── agents/
...
- **No real infrastructure is used.** All experiments run exclusively on synthetic datasets generated by ERENO-2.0 in a controlled local environment.
